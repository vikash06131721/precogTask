...
def add_embedding_columns(row):
    ...
    row["embedding_phrase1"] = get_embeddings(row["sentence1"])
    row["embedding_phrase2"] = get_embeddings(row["sentence2"])
    return row
...
plt.title("Lift Plot for Cosine Similarity Predictions For Gemma 2B for Sentence Similarity")
plt.xlabel("Proportion of Predictions (sorted by similarity)")
plt.ylabel("Lift")
plt.legend()
plt.grid()
plt.savefig("gemma2bSent.png")
